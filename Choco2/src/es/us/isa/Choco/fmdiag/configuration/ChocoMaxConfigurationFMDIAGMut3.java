...
public class ChocoMaxConfigurationFMDIAGMut3 extends ChocoQuestion implements ValidConfigurationErrorsQuestion {
...
*/
	...
	public int diagSource(List<String> D, List<String> P, List<String> AC) {	
		System.out.println("--- {" + D + "}, {" + P + "}");
		System.out.println("--- AC: " + AC + " - " + isConsistent(AC) + "\n");
		
		/*El orden de estas evaluaciones puede determinar la preferencia estilo-FastDiag del diagnóstico final*/
...
//		System.out.println(splitListToSubLists + "\n");
		...
		for (int i = splitListToSubLists.size() - 1; i >= 1; i--) {
			...
			resS.add(diagSource(Ds.get(j), sPrevious.get(j), ACs.get(j)));
			
			j++;
			
			if (resS.get(resS.size()-1) > 0)
				break;
		}
		...
		if (sPrevious.get(origen).size()>0 && resS.get(origen)== 3){	
